...
def get_random_email_content(cluster_id):
    ...
    df = pd.read_csv('junghee/main/project_1/data_mini1/클러스터링_이메일_수정.csv')
...
def send_promotion_email(이메일, 이름, cluster_id):
    ...
    msg.attach(MIMEText(body, "html"))
        ...
        # 로고 이미지 첨부 (CID 참조)
    with open("junghee/main/project_1/img/hyundai_logo.jpg", "rb") as img_file:
...
def send_welcome_email(이메일, 이름, 아이디, 가입일):
    ...
    if brand == '현대' :
            ...
            <table style="width: 100%; max-width: 800px; margin: auto; background: white; padding: 30px; 
                ...
                <tr>
                    ...
                    <td style="padding: 10px; text-align: right;">
                        <img src="cid:hyundai_welcome" alt="가입 환영" style="width: 200px; background: none; border: none;">
                    </td>
                </tr>
        ...
        # 웰컴 이미지 첨부 (CID 참조)
        with open("junghee/main/project_1/img/welcome.png", "rb") as img_file:
            img = MIMEImage(img_file.read())
            img.add_header("Content-ID", "<hyundai_welcome>")
            msg.attach(img)
        ...
        """기아자동차 회원가입 환영 이메일 자동 발송"""
        subject = "[기아자동차] 회원가입을 환영합니다! 🚗"
        ...
        <body style="font-family: 'Segoe UI', sans-serif; background-color: #fafafa; padding: 30px;">
            <table style="width: 100%; max-width: 800px; margin: auto; background: white; padding: 30px; 
                ...
                <tr>
                    <td style="padding: 10px; text-align: left;">
                        <a href="https://www.kia.com" target="_blank">
                            <img src="cid:kia_logo" alt="KIA 로고" style="width: 100px;">
                        </a>
                    </td>
                    <td style="padding: 10px; text-align: right;">
                        <img src="cid:kia_welcome" alt="환영 일러스트" style="width: 180px;">
                    </td>
                </tr>
                ...
                <tr>
                    <td colspan="2" style="padding: 30px; text-align: center;">
                        <h2 style="color: #000; margin-bottom: 10px;">WELCOME TO <span style="color: #b5121b;">KIA</span></h2>
                        <p style="font-size: 18px; color: #555;">기아자동차의 새로운 여정을 함께하게 되어 기쁩니다.</p>

                        <!-- 회원 정보 인사 -->
                        <div style="text-align: left; font-size: 17px; max-width: 600px; margin: 30px auto 20px;">
                            <p><strong>{이름}</strong> 고객님, 반갑습니다!</p>
                            <p>기아자동차에 가입해주셔서 감사합니다. 고객님의 라이프스타일에 맞는 특별한 서비스와 이벤트를 제공해드릴 예정입니다.</p>
                        </div>

                        <!-- 구분선 -->
                        <div style="width: 100%; max-width: 600px; margin: 25px auto; border-bottom: 1px solid #ccc;"></div>

                        <!-- 회원 정보 테이블 -->
                        <table style="width: 100%; max-width: 480px; margin: auto; border-collapse: collapse; background: #f1f1f1; 
                                    padding: 20px; border-radius: 10px; font-size: 16px;">
                            <tr>
                                <td style="padding: 10px; font-weight: bold;">아이디</td>
                                <td style="padding: 10px;">{아이디}</td>
                            </tr>
                            <tr>
                                <td style="padding: 10px; font-weight: bold;">가입일</td>
                                <td style="padding: 10px;">{가입일}</td>
                            </tr>
                        </table>

                        <!-- CTA 버튼 -->
                        <a href="https://www.kia.com"
                        style="display: inline-block; background: #b5121b; color: white; padding: 14px 28px; 
                                text-decoration: none; border-radius: 6px; margin-top: 30px; font-size: 16px;">
                            기아자동차 홈페이지 바로가기
                        </a>
                    </td>
                </tr>
                ...
                <!-- 푸터 -->
                <tr>
                    <td colspan="2" style="padding: 15px; font-size: 13px; text-align: center; color: #777;">
                        ※ 본 메일은 발신전용입니다. 자세한 문의는 고객센터를 이용해 주세요.
                    </td>
                </tr>
            </table>
        </body>
        ...
        # 로고 이미지 첨부 (CID 참조)
        with open("junghee/main/project_1/img/kia_logo.png", "rb") as img_file:
            img = MIMEImage(img_file.read())
            img.add_header("Content-ID", "<kia_logo>")
            msg.attach(img)

        # 웰컴 이미지 첨부 (CID 참조)
        with open("junghee/main/project_1/img/welcome_kia.png", "rb") as img_file:
            img = MIMEImage(img_file.read())
            img.add_header("Content-ID", "<kia_welcome>")
            msg.attach(img)
